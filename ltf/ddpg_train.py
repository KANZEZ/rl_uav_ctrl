...
pos_bound, vel_bound = 0.5, 0.2

model = DDPG(obs_dim, action_dim)
ac_obj = ActionContainer(action_dim)
observation = env.reset(seed=seed, initial_state='random', options={'pos_bound': pos_bound,
                                                                       'vel_bound': vel_bound})
...
for t in range(int(max_timesteps)):
    ...
    if t < critic_start_timesteps: # warmup to fill the buffer
        ...
        if (t+1) % critic_training_interval == 0:
            ...
            (obs, actions, ah, next_obs,
             rewards, dones) = model.get_batch(batch_size)
            model.train_critic(obs, actions, ah, next_obs, rewards, dones)
    ...
    next_observation, reward, done, _ = env.step(cur_action)
    ...
    if done:
        ...
        observation = env.reset(seed=seed, initial_state=reset_way, options={'pos_bound': pos_bound,
                                                                               'vel_bound': vel_bound})
        done = False
        episode_reward = 0
        episode_timesteps = 0
