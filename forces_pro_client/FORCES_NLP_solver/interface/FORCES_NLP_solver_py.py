...
a Python wrapper for a fast solver generated by FORCESPRO v6.3.0
   ...
   OUTPUT = FORCES_NLP_solver_py.FORCES_NLP_solver_solve(PARAMS) solves a multistage problem
   ...
   [OUTPUT, EXITFLAG] = FORCES_NLP_solver_py.FORCES_NLP_solver_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = FORCES_NLP_solver_py.FORCES_NLP_solver_solve(PARAMS) returns 
...
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'FORCES_NLP_solver/lib/FORCES_NLP_solver.so'))
    csolver = getattr(_lib,'FORCES_NLP_solver_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'FORCES_NLP_solver/lib/libFORCES_NLP_solver.so'))
    csolver = getattr(_lib,'FORCES_NLP_solver_solve')

internal_mem = getattr(_lib,'FORCES_NLP_solver_internal_mem')

class FORCES_NLP_solver_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('xinit', ctypes.c_double * 6),
('x0', ctypes.c_double * 168),
('all_parameters', ctypes.c_double * 21),
('reinitialize', ctypes.c_int),
]

FORCES_NLP_solver_params = {'xinit' : np.array([]),
'x0' : np.array([]),
...
FORCES_NLP_solver_params_types = {'xinit' : np.float64,
'x0' : np.float64,
'all_parameters' : np.float64,
'reinitialize' : np.int32,
}

class FORCES_NLP_solver_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
FORCES_NLP_solver_outputs = {'x01' : np.array([]),
'x02' : np.array([]),
'x03' : np.array([]),
'x04' : np.array([]),
'x05' : np.array([]),
...
class FORCES_NLP_solver_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
class FORCES_NLP_solver_mem(ctypes.Structure):
    pass
...
csolver.argtypes = ( ctypes.POINTER(FORCES_NLP_solver_params_ctypes), ctypes.POINTER(FORCES_NLP_solver_outputs_ctypes), ctypes.POINTER(FORCES_NLP_solver_info), ctypes.POINTER(FORCES_NLP_solver_mem), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

internal_mem.argtypes = [ctypes.c_uint]
internal_mem.restype = ctypes.POINTER(FORCES_NLP_solver_mem)

def FORCES_NLP_solver_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCESPRO v6.3.0
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = FORCES_NLP_solver_params_ctypes()
    for par in params_arg:
        ...
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=FORCES_NLP_solver_params_types[par], requirements='F')
    ...
    outputs_py = FORCES_NLP_solver_outputs_ctypes()
    info_py = FORCES_NLP_solver_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.FORCES_NLP_solver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), PyFile_AsFile(fp)  )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.FORCES_NLP_solver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), None )
            else:
                exitflag = _lib.FORCES_NLP_solver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), fp )
            libc.fclose(fp)
    ...
    # convert outputs
    for out in FORCES_NLP_solver_outputs:
        FORCES_NLP_solver_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return FORCES_NLP_solver_outputs,int(exitflag),info_py

solve = FORCES_NLP_solver_solve
