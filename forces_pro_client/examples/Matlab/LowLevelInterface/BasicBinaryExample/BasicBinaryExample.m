...
% (c) Embotech AG, Zurich, Switzerland, 2015-2023.
    ...
    clcost = 0;
    prepadzeros = num2str(ceil(log10(N-1)));
    ...
    exitflag  = -ones(1,kmax);
    solvetime = -ones(1,kmax);
    qpsolves  = -ones(1,kmax);
    it2opt    = -ones(1,kmax);
    ...
    if( doplot.exitflags )
        figure(3); clf;
        histData = hist(exitflag,[-2,-1,0,1,2]-0.5);
        bar([-2,-1,0,1,2],histData/length(exitflag)*100);
