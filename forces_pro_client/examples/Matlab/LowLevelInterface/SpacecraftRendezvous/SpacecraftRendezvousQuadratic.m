function [] = SpacecraftRendezvousQuadratic()
...
%       Ed Hartley, Cambridge University, UK.
    ...
    for k = 1:kmax
        ...
        if mod(k-1,Ts_MPC) == 0 % only run the controller every Ts_MPC seconds
            ...
            problem.D_1 = [B, -eye(nx)];
            for i = 2:N-1
                [A, nucopy] = YA_A_matrix(cfg_MPC, nucopy, Ts_MPC*Ts_SIM);
                B = A*[zeros(3);eye(3)];
                problem.(['D_',num2str(i)]) = [B, -eye(nx)];
                problem.(['C_',num2str(i-1)]) = [zeros(nx,nu), A];
            end   

            [solverout,exitflag,~] = FORCESPRO_spacecraft_controller(problem);   % solve
            if( exitflag == 1 )
                U(:,k) = solverout.u0;
            else
