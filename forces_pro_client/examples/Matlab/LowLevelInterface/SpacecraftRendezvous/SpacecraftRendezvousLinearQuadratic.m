function [] = SpacecraftRendezvousLinearQuadratic()
...
%       Ed Hartley, Cambridge University, UK.
    ...
    % assume variable ordering z(i) = [ui, x_i+1, eu_i] for i=1...N-1
    ...
    for k = 1:kmax
        ...
        if mod(k-1,Ts_MPC) == 0 % only run the controller every Ts_MPC seconds
            ...
            problem.D_1 = [B, -eye(nx), zeros(nx,nu)]; 
            for i = 2:N-1
                [A, nucopy] = YA_A_matrix(cfg_MPC, nucopy, Ts_MPC*Ts_SIM);
                B = A*[zeros(3);eye(3)];
                problem.(['D_',num2str(i)]) = [B, -eye(nx), zeros(nx,nu)];
                problem.(['C_',num2str(i-1)]) = [zeros(nx,nu), A, zeros(nx,nu)];
            end   
