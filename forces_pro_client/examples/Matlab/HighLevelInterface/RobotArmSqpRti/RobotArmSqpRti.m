...
%
% It also shows how to generate a MEX entry point for the nonlinear
% dynamics of the robot which can be used directly for simulation and
% debugging in MATLAB.
%
...
% (c) Embotech AG, Zurich, Switzerland, 2013-2023.
    ...
    codeoptions.MEXinterface.dynamics = 1; % Generate a MEX entrypoint for the nonlinear dynamics of the robot
    ...
    for i = 1:Ns/2
        ...
        % Simulate dynamics
        z = [U(:,i); X(:,i)];
        p = problem.all_parameters(1,:);
        [c, ~] = RobotArmSolver_dynamics(z,p); % MEX entry point for nonlinear dynamics

        % Compute closed-loop cost
        cost(i) = codeoptions.nlp.integrator.Ts*(c'*Q*c + U(:,i)'*R*U(:,i));
