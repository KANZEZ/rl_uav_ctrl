...
%
% The example can be run with two different algorithms: The QP "general"
% algorithm and the "QP fast" algorithm.
%
% Guide for "QP general" algorithm:
%   1) run 'forcesMpc_miso()' in your Matlab terminal.
%
% Guide for "QP fast" algorithm:
%   The "QP fast" algorithm tunes certain algorithm parameters according to
%   the problem formulation for which it is designed. To do this one has to
%   first collect data for tuning the solver. Collecting data is done using
%   the following 3 commands:
%       1) run 'forcesMpcCache on' in your Matlab terminal.
%       2) run 'forcesMpc_miso()' in your Matlab terminal.
%       3) run 'forcesMpcCache off' in your Matlab terminal.
%   After data has been collected you can run the "QP fast" algorithm as
%   follows:
%       1) run 'forcesMpc_miso()' in your Matlab terminal.
%
