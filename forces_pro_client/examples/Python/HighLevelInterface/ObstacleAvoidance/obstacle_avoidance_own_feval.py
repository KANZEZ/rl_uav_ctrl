...
def generate_pathplanner():
    ...
    # are imposed
    model.bfgs_init = 3.0*np.identity(7) # initialization of the 
...
def calculate_path(event,model,solver,problem):
    ...
    if (params[0] == None) or (params[1] == None):
        ...
        if exitflag < 0:
            ...
            update_plots(x,u,model,params)

def update_plots(x,u,model,params):
...
def plot_and_make_interactive(x,u,model,params,problem,solver):
...
def main():
    ...
    plot_and_make_interactive(x,u,model,params,problem,solver)
