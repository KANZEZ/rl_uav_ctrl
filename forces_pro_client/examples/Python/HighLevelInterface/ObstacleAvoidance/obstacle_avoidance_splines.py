...
from forcespro.modelling import Interpolation, InterpolationFit
...
def setup_road_limit_splines():
    """
    Returns two cubic splines representing the upper and lower limit
    of the road.
    """

    x_pos   = np.array([-3.0, -2.0, -1.0, 0.0])
    y_upper = np.array([ 0.0,  1.8,  2.5, 3.0])
    y_lower = np.array([-5.0, -2.0,  0.3, 1.5])

    upper_limit_spline = InterpolationFit( x_pos,y_upper )
    lower_limit_spline = InterpolationFit( x_pos,y_lower )

    return upper_limit_spline, lower_limit_spline
...
def generate_pathplanner():
    ...
    # You can use standard Python handles to define these functions, i.e. you
    # could also have this in a separate file. We use anonymous handles here
    # only for convenience. The function must be able to handle symbolic evaluation,
    ...
    # obstacle
    upper_road_limit, lower_road_limit = setup_road_limit_splines()
    model.ineq = lambda z,p:  casadi.vertcat(z[3] - upper_road_limit(z[2]),
                                             lower_road_limit(z[2]) - z[3],
                                             (z[2] -p[0]) ** 2 + (z[3] - p[1]) ** 2)

    # Upper/lower bounds for inequalities
    model.hu = np.array([      0,       0, +np.inf])
    model.hl = np.array([-np.inf, -np.inf,  0.6**2])
...
def update_plots(x,u,model,params):
    ...
    ellipse((params[0]-np.sqrt(model.hl[2]), \
        params[1]-np.sqrt(model.hl[2]), \
        2*np.sqrt(model.hl[2]), \
        2*np.sqrt(model.hl[2])), \
...
def plot_and_make_interactive(x,u,model,params,problem,solver):
    ...
    x_start, x_end = upper_road_limit.get_range()
    x_pos_fine = np.linspace( x_start,x_end,100 )
    upper_road_limit_fine = upper_road_limit( x_pos_fine )
    lower_road_limit_fine = lower_road_limit( x_pos_fine )

    plt.plot(x_pos_fine, upper_road_limit_fine, 'r:')
    plt.plot(x_pos_fine, lower_road_limit_fine, 'r:')
    ...
    ellipse((params[0]-np.sqrt(model.hl[2]), params[1]-np.sqrt(model.hl[2]), \
        2*np.sqrt(model.hl[2]), 2*np.sqrt(model.hl[2])), fill=False, linestyle=":")
