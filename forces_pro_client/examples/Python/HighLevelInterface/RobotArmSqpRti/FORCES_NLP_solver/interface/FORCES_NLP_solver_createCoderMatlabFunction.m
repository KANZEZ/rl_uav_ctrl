...
% 
% FORCES_NLP_solver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the FORCES_NLP_solver FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% FORCES_NLP_solver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% FORCES_NLP_solver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% FORCES_NLP_solver_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function FORCES_NLP_solver_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'FORCES_NLP_solver_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'FORCES_NLP_solver_block';
    end
    ...
    function_name = 'FORCES_NLP_solver_coderFunction.m';
    position = [170, 99, 650, 537];
    parameter_sizes = struct('xinit', '[6,1]',...
                             'x0', '[168,1]',...
                             'all_parameters', '[21,1]',...
                             'reinitialize', '[1,1]');
    if useCompactBlock
        function_name = 'FORCES_NLP_solver_coderFunctioncompact.m';
