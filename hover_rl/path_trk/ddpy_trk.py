"""
todo:
1. Implement the DDPG algorithm in rotorpy/learning/td3.py, train a hover policy.
...
class Actor(nn.Module): # bzs = 256
    ...
    def forward(self, obs, ah, fu):
        a = torch.cat([obs, ah, fu], 1)
        a = F.tanh(self.l1(a))
        a = F.tanh(self.l2(a))
...
class DDPG(object):
    def __init__(self, obs_dim, action_dim, discount=0.99, tau=0.005, max_ah_size=32, future_traj_dim=20):
        """
        obs_dim: observation dimension, p, v, q, w (13)
        action_dim: action dimension, 4 (motors speed)
        """
        self.actor = Actor(obs_dim+action_dim*max_ah_size+3*future_traj_dim, action_dim).to(device)
    ...
    def train(self, batch_size, train_mode):
        ...
        """
        obs, action, ah, next_obs, reward, done, fu = self.get_batch(batch_size)
        with torch.no_grad():
            noise = (torch.randn_like(action) * self.act_tar_noise
                     ).clamp(-self.clip, self.clip)
            next_ah = torch.cat([ah[:, 4:], action], 1)
            next_act = self.actor_target(next_obs, next_ah, fu) + noise
        ...
        if train_mode == 1:
            # Compute actor loss
            actor_loss = -self.critic.Q1(obs, self.actor(obs, ah, fu)).mean()
            # Optimize the actor
