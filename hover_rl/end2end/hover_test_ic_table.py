...
def get_initial_cond():
    ...
    px = random_sign() * np.random.uniform(plb, pub)
    py = random_sign() * np.random.uniform(plb, pub)
    pz = random_sign() * np.random.uniform(plb, pub)

    vx = random_sign() * np.random.uniform(vlb, vub)
    vy = random_sign() * np.random.uniform(vlb, vub)
    vz = random_sign() * np.random.uniform(vlb, vub)

    wx = random_sign() * np.random.uniform(wlb, wub)
    wy = random_sign() * np.random.uniform(wlb, wub)
    wz = random_sign() * np.random.uniform(wlb, wub)

    w = np.array([wx, wy, wz])
    q = Rotation.from_euler('zyx', np.random.uniform(low=np.pi/3, high=np.pi/2, size=(3,))).as_quat()

    return {'x': np.array([px, py, pz]),
             'v': np.array([vx, vy, vz]),
...
small: 0.1 /0.1
medium: 0.1 /0.1 
big: 0.4 / 0.1 ### it's fair to increase the threshold with big wind
"""
def if_reach(obs, t):
    if np.linalg.norm(obs[:3]) < 0.1 and np.linalg.norm(obs[3:6]) < 0.1:
        return True, t
    return False, t
...
if __name__ == "__main__":
    ...
    for iter in range(max_iter):
        ...
        for (k, num_timesteps_idx) in enumerate(num_timesteps_idxs):  # For each num_timesteps index...
            ...
            initial_state = get_initial_cond()
