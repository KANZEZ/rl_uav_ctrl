...
def small_random_wind():
    ...
    flb = 1
    fub = 2
    ax = random_sign() * np.random.uniform(alb, aub)
    ay = random_sign() * np.random.uniform(alb, aub)
    az = random_sign() * np.random.uniform(alb, aub)
    fx = random_sign() * np.random.uniform(flb, fub)
    fy = random_sign() * np.random.uniform(flb, fub)
    fz = random_sign() * np.random.uniform(flb, fub)
    return SinusoidWind(amplitudes=np.array([ax, ay, az]), frequencies=np.array([fx, fy, fz]))
...
"""
def if_reach(obs, t):
    if np.linalg.norm(obs[:3]) < 0.2 and np.linalg.norm(obs[3:6]) < 0.1:
        return True, t
    return False, t
